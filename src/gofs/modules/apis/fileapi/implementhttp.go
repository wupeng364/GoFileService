...
func (this *imp_http) init( ){
	// 批量注册Handle
	err := this.hs.AddRegistrar(this)
	if err != nil { panic(err) }
	
	// 单个注册Handle
	err = this.hs.AddHandlerFunc(base_url+"/upload/:"+`[\S]+`, this.Upload)
	if err != nil { panic(err) }
	err = this.hs.AddHandlerFunc(base_url+"/download/:"+`[\S]+`, this.Download)
	if err != nil { panic(err) }
	err = this.hs.AddHandlerFunc(base_url+"/openfile/:"+`[\S]+`, this.OpenFile)
	if err != nil { panic(err) }
	
	// 注册Api签名拦截器
	this.hs.AddIgnoreFilter(base_url+"/download/:"+`[\S]+`)
	this.hs.AddIgnoreFilter(base_url+"/openfile/:"+`[\S]+`)
	this.hs.AddIgnoreFilter(base_url+"/upload/:"+`[\S]+`)
	this.hs.AddUrlFilter(base_url+"/:"+`[\S]+`, this.sg.ApiFilter_Http)
	
	fmt.Println("   > FileApiModule http registered end")
}

// 向 Server Router 中注册下列处理器 , 实现接口 httpserver.Registrar
func (this *imp_http) RoutList( ) httpserver.StructRegistrar{
	return httpserver.StructRegistrar{
		IsToLower: true,
		BasePath: base_url,
		HandlerFunc: []hst.HandlersFunc{
			this.TransferToken,
			this.BatchOperationTokenStauts,
			this.List,
			this.Del,
			this.DelVer,
			this.ReName,
...
// 传输令牌申请
func (this *imp_http) TransferToken( w http.ResponseWriter, r *http.Request ){
	q_path := r.FormValue("path")
	if len( q_path ) == 0 {
		sendError(w, Error_OprationFailed); return
	}
	if !this.fm.IsFile(q_path) {
		sendError(w, Error_FileNotExist); return
	}
	sendSuccess(w, this.fm.AskToken(TokenType.download, &FileTransferToken{
		FilePath: q_path,
	}))
}
// Token信息查询 Get用于查询|Post用于操作(ErrorOperation)
func (this *imp_http) BatchOperationTokenStauts( w http.ResponseWriter, r *http.Request ){
	q_token := r.FormValue("token")
	this.fm.RefreshToken(q_token)
	tokenErr, tokenBody := getFileBatchOperationTokenObject(this, q_token)
...
// 查询路径下的列表以及基本信息
func (this *imp_http) List( w http.ResponseWriter, r *http.Request ){
	q_path := r.FormValue("path")
	...
	res, err := this.fm.GetDirListInfo(q_path)
	if err != nil {
		sendError(w, err ) 
		return
	}
...
// 批量|单个删除文件|文件夹
func (this *imp_http) Del( w http.ResponseWriter, r *http.Request ){
	q_path := r.FormValue("path")
	if len(q_path) == 0 {
		sendError(w, errors.New("'Path' parameter not found")); return
	}
	if !this.fm.IsExist(q_path) {
		sendError(w, Error_FileNotExist); return
	}
	del_err := this.fm.DoDelete(q_path)
	if nil == del_err {
		sendSuccess(w, "")
	}else{
		sendError(w, del_err)
...
func (this *imp_http) MoveAsync(w http.ResponseWriter, r *http.Request){
	...
	token := this.fm.AskToken(TokenType.moveDir, &moveTokenObject)
	go func ( token string) {
		moveDirErr := this.fm.DoMove(q_src_path, q_dst_path, q_replace, q_ignore, func(count int, s_src, s_dst string, moveErr *filemanage.MoveError)error{
			// 获取令牌数据, 不存在则说明已经销毁
			// 并保持刷新token的有效性, 除非终止操作否则都继续
			tokenErr, tokenBody := getFileBatchOperationTokenObject(this, token)
			if nil != tokenErr {
				return tokenErr
			}
			...
			if nil != moveErr {
				...
				if moveErr.IsDstExist {
					...
					if tokenBody.IsReplaceAll {
						...
						reMoveErr := this.fm.DoMove(s_src, s_dst, true, false, func(c int, s, d string, mErr *filemanage.MoveError)error{
							if nil != mErr {
				...
				for{
					...
					if tokenBody.IsReplace || tokenBody.IsReplaceAll {
						...
						if moveErr.IsSrcExist {
							moveCopyErr := this.fm.DoMove(s_src, s_dst, true, false, func(c int, s, d string, mErr *filemanage.MoveError)error{ 
								if nil != mErr {
									return errors.New(mErr.ErrorString)
								}
								return nil
		...
		// 到这里如果没有错误就是成功了
		tokenObject := this.fm.GetToken(token)
...
func (this *imp_http) CopyAsync( w http.ResponseWriter, r *http.Request ){
	q_src_path := r.FormValue("srcPath")
	q_dst_path := r.FormValue("dstPath")
	q_replace  := stringtools.ParseBool( r.FormValue("replace") )
	q_ignore   := stringtools.ParseBool( r.FormValue("ignore") )
	...
	token := this.fm.AskToken(TokenType.copyDir, &copyTokenObject)
	go func ( token string ) {
		// 这里面已经不属于一个会话, 使用令牌保存数据
		copyDirErr := this.fm.DoCopy(q_src_path, q_dst_path, q_replace, q_ignore, func(count int, s_src, s_dst string, copyErr *filemanage.CopyError)error{
			// 获取令牌数据, 不存在则说明已经销毁
			// 并保持刷新token的有效性, 除非终止操作否则都继续
			tokenErr, tokenBody := getFileBatchOperationTokenObject(this, token)
			if nil != tokenErr {
			...
			if nil != copyErr {
				...
				if copyErr.IsDstExist {
					...
					if tokenBody.IsReplaceAll {
						// 先删除然后再替换, 如果覆盖操作没有出现问题
						reCopyErr := this.fm.DoCopy(s_src, s_dst, true, false, func(c int, s, d string, cErr *filemanage.CopyError)error{
							if nil != cErr {
...
func (this *imp_http) DelVer( w http.ResponseWriter, r *http.Request ){
	
}
// 重命名
func (this *imp_http) ReName( w http.ResponseWriter, r *http.Request ){
	q_src_path := r.FormValue("path")
	q_name := r.FormValue("name")
	if !this.fm.IsExist( q_src_path ){
		sendError(w, Error_FileNotExist); return;
	}
	if len(q_name) == 0 {
		sendError(w, Error_NewNameIsEmpty); return;
	}
	rnm_err := this.fm.DoRename(q_src_path, q_name)
	if nil == rnm_err {
		sendSuccess(w, "")
	}else{
		sendError(w, rnm_err);
...
// 新建文件夹
func (this *imp_http) NewFolder( w http.ResponseWriter, r *http.Request ){
	q_src_path := r.FormValue("path")
	q_src_path = stringtools.ClerPath( q_src_path )
	if !this.fm.IsExist( stringtools.GetParentPath( q_src_path ) ){
		sendError(w, Error_ParentFolderNotExist); return;
	}
	rnm_err := this.fm.DoNewFolder(q_src_path)
	if nil == rnm_err {
		sendSuccess(w, "")
	}else{
		sendError(w, rnm_err);
...
func (this *imp_http) Info( w http.ResponseWriter, r *http.Request ){
	
}
func (this *imp_http) NameSearch( w http.ResponseWriter, r *http.Request ){
	
}
// 文件上传, 支持Form和Body上传方式
// 参数: Header("Save-Path", ["Formname-File"])
func (this *imp_http) Upload( w http.ResponseWriter, r *http.Request ){
	...
	if err == nil{
		...
		err := this.fm.DoWrite(dst, r.Body)
		if nil != err {
			sendError(w, err)
		}else{
			sendSuccess(w, "")
...
// 打开
func (this *imp_http) OpenFile( w http.ResponseWriter, r *http.Request ){
	token := stringtools.GetPathName( r.URL.Path )
	_index := strings.Index(token, ".")
	if _index > -1 {
		token = token[:_index]
	}
	err, tokenObject := getFileTransferTokenObject(this, token)
	if nil != err || nil == tokenObject {
		sendError(w, Error_OprationExpires); return
	}
	//this.fm.RemoveToken(token)
	rd, r_err := this.fm.DoRead(tokenObject.FilePath)
	if r_err != nil {
...
// 下载
func (this *imp_http) Download( w http.ResponseWriter, r *http.Request ){
	token := stringtools.GetPathName( r.URL.Path )
	err, tokenObject := getFileTransferTokenObject(this, token)
	if nil != err || nil == tokenObject {
		sendError(w, Error_OprationExpires); return
	...
	fileSize, fs_size_err := this.fm.GetFileSize(tokenObject.FilePath)
	if nil != fs_size_err {
...
// 获取批文件量操作Token对象
func getFileBatchOperationTokenObject( this *imp_http, token string )(error, *FileBatchOperationTokenObject){
	tokenObject := this.fm.GetToken(token)
	// 并保持刷新token的有效性, 除非终止操作否则都继续
	if nil == tokenObject || nil == tokenObject.TokenBody {
...
// 获取文件传输Token对象
func getFileTransferTokenObject( this *imp_http, token string )(error, *FileTransferToken){
	tokenObject := this.fm.GetToken(token)
	// 并保持刷新token的有效性, 除非终止操作否则都继续
	if nil == tokenObject || nil == tokenObject.TokenBody {
		return Error_Discontinue, nil
