...
// 初始化挂载节点
func (this *MountManager) initMountItems( mounts map[string]interface{} ) *MountManager{
	if len(mounts) == 0 {
		panic("mounts is nil")
	}
...
// 根据相对路径获取对应驱动类
func (this *MountManager) getInterface( relativePath string) fmInterface {
	if len(strings.Replace(relativePath, " ", "", -1)) == 0 {
		relativePath = "/"
	}
	// 挂载节点
	recentMtItems := this.getMountItem(relativePath)
	// 解析 recentMtItems
	...
	switch recentMtItems.MtType {
		case mt_type_local:{ // 本地存储
			return &imp_local{ recentMtItems, this }
			break
		}
...
// 查找相对路径下的分区挂载信息
func (this *MountManager) getMountItem( relativePath string ) MtItems{
	// 如果传入路径和挂载节点匹配, 则记录下来
	_PathLen := -1
	var recentMtItems MtItems
	for _, _val := range this.Mts{
...
func (this *MountManager) findMountChild( relativePath string ) (res []string){
	if relativePath != "/" {
		return res
	}
