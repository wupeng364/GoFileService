 ...
 * 文件是否存在
**/
func (this imp_local) IsExist( relativePath string ) (bool, error){
	_abs_p, _, err := getAbsolutePath(this.MountInfo, relativePath)
    return filetools.IsExist( _abs_p ), err
}
func (this imp_local) IsDir( relativePath string ) (bool, error){
	_abs_p, _, err := getAbsolutePath(this.MountInfo, relativePath)
    return filetools.IsDir( _abs_p ), err
}
func (this imp_local) IsFile( relativePath string ) (bool, error){
	_abs_p, _, err := getAbsolutePath(this.MountInfo, relativePath)
    return filetools.IsFile( _abs_p ), err
}
func (this imp_local) GetDirList( relativePath string ) ([]string, error){
	_abs_p, _rl_p, err := getAbsolutePath(this.MountInfo, relativePath)
	if err != nil {
...
func (this imp_local) GetFileSize( relativePath string ) (int64, error){
	_abs_p, _, err := getAbsolutePath(this.MountInfo, relativePath)
    return filetools.GetFileSize( _abs_p ), err
}
func (this imp_local) GetCreateTime( relativePath string ) (int64, error){
	_abs_p, _, err := getAbsolutePath(this.MountInfo, relativePath)
    return filetools.GetCreateTime( _abs_p ).UnixNano( ) / 1e6, err
}
func (this imp_local) GetModifyTime( relativePath string ) (int64, error){
	_abs_p, _, err := getAbsolutePath(this.MountInfo, relativePath)
    return filetools.GetModifyTime( _abs_p ).UnixNano( ) / 1e6, err
}
func (this imp_local) GetCreateUser( relativePath string ) (string, error){
	return "", nil
}
func (this imp_local) GetModifyUser( relativePath string ) (string, error){
	return "", nil
}
func (this imp_local) GetFileLatestVersion( relativePath string ) (string, error){
	return "", nil
}
func (this imp_local) GetFileVersionList( relativePath string ) ([]string, error){
	return []string{ }, nil
}
// 移动文件|夹 
func (this imp_local) DoMove(src string, dst string, replace, ignore bool, callback MoveCallback)error{
	if this.MountInfo.MtPath == src {
		return errors.New("Does not allow access: "+ src)
	}
	_abs_src, _, _err := getAbsolutePath(this.MountInfo, src)
	if nil != _err {
		return _err
	}
	if filepath.Clean(this.MountInfo.MtAddr) == _abs_src {
		return errors.New(src+" is mount root, cannot move")
	}
	// 目标位置驱动接口
	dstMountItem := this.mtm.getMountItem(dst)
	...
	switch dstMountItem.MtType {
		case mt_type_local:{ // 本地存储
			return filetools.MoveDir(_abs_src, _abs_dst, replace, ignore, func(count int, s_src, s_dst string, err error)error{
				_r_src := getRelativePath(this.MountInfo, s_src)
				...
				if nil != err {
					...
					return callback(count, _r_src, _r_dst, &MoveError{
								...
								ErrorString: parseErrorString(this.MountInfo.MtAddr, dstMountItem.MtAddr, err),
							})
				}
				return callback(count, _r_src, _r_dst, nil)
			})
...
// 重命名文件|文件夹
func (this imp_local) DoRename(relativePath string, newName string)error{
	if this.MountInfo.MtPath == relativePath {
		return errors.New("Does not allow access: "+ relativePath)
	}
	_abs_src, _, _err := getAbsolutePath(this.MountInfo, relativePath)
	if nil != _err {
		return _err
	}
	if len(newName) == 0 {
...
// 新建文件夹
func (this imp_local) DoNewFolder(relativePath string)error{
	if this.MountInfo.MtPath == relativePath {
		return errors.New("Does not allow access: "+ relativePath)
	}
	_abs_src, _, _err := getAbsolutePath(this.MountInfo, relativePath)
...
// 删除文件|文件夹
func (this imp_local) DoDelete( relativePath string )error{
	if this.MountInfo.MtPath == relativePath {
		return errors.New("Does not allow access: "+ relativePath)
	}
	_abs_src, _, _err := getAbsolutePath(this.MountInfo, relativePath)
	...
	_deleting_path := getAbsoluteDeletingPath(this.MountInfo)
	// 移动到删除零时目录, 如果存在则覆盖
	...
	if nil == mv_err {
		go this.DoClearDeletings( )
	}
	return mv_err
}
// 删除各个分区内的'临时删除文件'
func (this imp_local) DoClearDeletings( ){
	for _, _val := range this.mtm.Mts{
...
// 拷贝文件
func (this imp_local) DoCopy(src, dst string, replace, ignore bool, callback CopyCallback)error{
	_abs_src, _, _err := getAbsolutePath(this.MountInfo, src)
	if nil != _err {
		return _err
	}
	...
	switch dstMountItem.MtType {
		case mt_type_local:{ // 本地存储
			if filetools.IsFile(_abs_src){
				_r_src := getRelativePath(this.MountInfo, _abs_src)
				...
				if nil != err {
					return callback(1, _r_src, _r_dst, &CopyError{
								...
								ErrorString: parseErrorString(this.MountInfo.MtAddr, dstMountItem.MtAddr, _err),
							})
				}
...
func (this imp_local) DoCreat( )(bool, error){
	return true, nil
}
// 读取文件
func (this imp_local) DoRead( relativePath string )(io.Reader, error){
	abs_dst, _, gp_err := getAbsolutePath(this.MountInfo, relativePath)
	if nil != gp_err {
		return nil, gp_err
	}
...
// 写入文件
// 先写入临时位置, 然后移动到正确位置
func (this imp_local) DoWrite( relativePath string, ioReader io.Reader ) error{
	if ioReader == nil {
	...
	tempPath := getAbsoluteTempPath(this.MountInfo)
...
func (this imp_local) AskUploadToken( relativePath string )(string, error){
	return "", nil
}
func (this imp_local) SaveTokenFile(token string, src io.Reader)(bool){
	return true
}
func (this imp_local) SubmitToken(token string, isCreateVer bool)(bool, error){
	return true, nil
}
