...
func (fa *imp_http) init( ){
	// 批量注册Handle
	err := fa.hs.AddRegistrar(fa)
	if err != nil { panic(err) }
	
	// 单个注册Handle
	err = fa.hs.AddHandlerFunc(base_url+"/upload/:"+`[\S]+`, fa.Upload)
	if err != nil { panic(err) }
	err = fa.hs.AddHandlerFunc(base_url+"/download/:"+`[\S]+`, fa.Download)
	if err != nil { panic(err) }
	err = fa.hs.AddHandlerFunc(base_url+"/openfile/:"+`[\S]+`, fa.OpenFile)
	if err != nil { panic(err) }
	
	// 注册Api签名拦截器
	fa.hs.AddIgnoreFilter(base_url+"/download/:"+`[\S]+`)
	fa.hs.AddIgnoreFilter(base_url+"/openfile/:"+`[\S]+`)
	fa.hs.AddIgnoreFilter(base_url+"/upload/:"+`[\S]+`)
	fa.hs.AddUrlFilter(base_url+"/:"+`[\S]+`, fa.sg.ApiFilter_Http)
