...
// 初始化驱动
func (slt *imp_sqlite)InitDriver(db interface{}) error{
	if nil == db {
		return Error_ConnIsNil
	}
	slt.db = db.(*sqlite.SqliteModule)
	return nil
}

// 初始化 users 表
// 添加默认用户 admin, 密码为空
func (slt *imp_sqlite)InitTables( )error{
	// 打开数据库
	db, err := slt.db.Open( )
	defer db.Close( )
...
// 列出所有用户数据, 无分页
func (slt *imp_sqlite)ListAllUsers( )(*[]UserInfo, error) {
	// 打开数据库
	db, err := slt.db.Open( )
	defer db.Close( )
...
// 根据用户ID查询详细信息
func (slt *imp_sqlite)QueryUser(userId string)(*UserInfo, error) {
	// 打开数据库
	db, err := slt.db.Open( )
	defer db.Close( )
	if err != nil { return nil, err }
...
// 添加用户
func (slt *imp_sqlite)AddUser(user *UserInfo) error {
...
// 修改用户
func (slt *imp_sqlite)UpdateUser(user *UserInfo) error {
	if len(user.UserId) == 0 {
		return Error_UserIdIsNil
	}
	
	// 查询旧数据 - 不校验
//	user_old, err := slt.QueryUser(user.UserId)
...
// 根据userId删除用户
func (slt *imp_sqlite)DelUser(userId string) error {
	if len(userId) == 0 {
		return Error_UserIdIsNil
	}
...
// 校验密码是否一致
func (slt *imp_sqlite)CheckPwd(userId, pwd string) bool {
	if len(userId) == 0 {
		return false
	}
